...
POCKET SYNTH targets modern browsers and prioritizes the complete synth, sampler, loop, sequencer, project, and audio feature set.
...
Do not disable, rewrite, or constrain modern POCKET SYNTH functionality merely to preserve support for obsolete browsers. When a required Web API is unavailable, unsupported browsers may fail the capability requirement rather than forcing the main application into a degraded compatibility architecture.
